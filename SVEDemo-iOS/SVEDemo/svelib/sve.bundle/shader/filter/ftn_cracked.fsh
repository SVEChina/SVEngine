...
void main() {
    ...
    for(int c=0;c<30;c++){
        ...
        vec2 vc=vec2(center.x+cos(angle)*dist+rnd(vec2(float(c),349.3))*7E-3,center.y+sin(angle)*dist+rnd(vec2(float(c),912.7))*7E-3);
    ...
    if(col<.3)
        col=0.;
    vec4 tex=texture2D(aTexture0,(fragCoord.xy)/iResolution.xy+rnd(v)*.02);
