...
#ifndef SV_TRIANGLE_3D_H
#define SV_TRIANGLE_3D_H

#include "SVVector3d.h"
#include "SVLine3d.h"
#include "SVPlane3d.h"
#include "SVaabbox3d.h"
...
namespace sv{
    ...
    namespace util {
        ...
        public:
            ...
            SVTriangle3d(const SVVector3d<T> &v1, const SVVector3d<T> &v2, const SVVector3d<T> &v3)
            : pointA(v1), pointB(v2), pointC(v3) {}
            
            //! Equality operator
            bool operator==(const SVTriangle3d<T> &other) const {
            ...
            bool isTotalInsideBox(const SVaabbox3d <T> &box) const {
            ...
            bool isTotalOutsideBox(const SVaabbox3d <T> &box) const {
            ...
            SVVector3d <T> closestPointOnTriangle(const SVVector3d <T> &p) const {
                const SVVector3d <T> rab = SVLine3d<T>(pointA, pointB).getClosestPoint(p);
                const SVVector3d <T> rbc = SVLine3d<T>(pointB, pointC).getClosestPoint(p);
                const SVVector3d <T> rca = SVLine3d<T>(pointC, pointA).getClosestPoint(p);
            ...
            bool isPointInside(const SVVector3d<T> &p) const {
                SVVector3d<f64> af64((f64) pointA.X, (f64) pointA.Y, (f64) pointA.Z);
                SVVector3d<f64> bf64((f64) pointB.X, (f64) pointB.Y, (f64) pointB.Z);
                SVVector3d<f64> cf64((f64) pointC.X, (f64) pointC.Y, (f64) pointC.Z);
                SVVector3d<f64> pf64((f64) p.X, (f64) p.Y, (f64) p.Z);
            ...
            bool isPointInsideFast(const SVVector3d<T> &p) const {
                const SVVector3d<T> a = pointC - pointA;
                const SVVector3d<T> b = pointB - pointA;
                const SVVector3d<T> c = p - pointA;
            ...
            bool getIntersectionWithLimitedLine(const SVLine3d <T> &line,
                                                SVVector3d<T> &outIntersection) const {
            ...
            bool getIntersectionWithLine(const SVVector3d<T> &linePoint,
                                         const SVVector3d<T> &lineVect,
                                         SVVector3d<T> &outIntersection) const {
            ...
            bool getIntersectionOfPlaneWithLine(const SVVector3d<T> &linePoint,
                                                const SVVector3d<T> &lineVect,
                                                SVVector3d<T> &outIntersection) const {
                // Work with f64 to get more precise results (makes enough difference to be worth the casts).
                const SVVector3d<f64> linePointf64(linePoint.X, linePoint.Y, linePoint.Z);
                const SVVector3d<f64> lineVectf64(lineVect.X, lineVect.Y, lineVect.Z);
                SVVector3d<f64> outIntersectionf64;
                
                SVTriangle3d <f64> trianglef64(
                                               SVVector3d<f64>((f64) pointA.X, (f64) pointA.Y, (f64) pointA.Z),
                                               SVVector3d<f64>((f64) pointB.X, (f64) pointB.Y, (f64) pointB.Z),
                                               SVVector3d<f64>((f64) pointC.X, (f64) pointC.Y, (f64) pointC.Z));
                const SVVector3d<f64> normalf64 = trianglef64.getNormal().normalize();
                f64 t2;
            ...
            bool isFrontFacing(const SVVector3d<T> &lookDirection) const {
                const SVVector3d<T> n = getNormal().normalize();
                const f32 d = (f32) n.dotProduct(lookDirection);
                return F32_LOWER_EQUAL_0(d);
            }
            
            //! Get the plane of this triangle.
            SVPlane3d <T> getPlane() const {
                return SVPlane3d<T>(pointA, pointB, pointC);
            }
            ...
            void set(const SVVector3d <T> &a, const SVVector3d <T> &b,
                     const SVVector3d <T> &c) {
                pointA = a;
                pointB = b;
                pointC = c;
            }
        ...
        private:
            ...
            bool isOnSameSide(const SVVector3d<f64> &p1, const SVVector3d<f64> &p2,
                              const SVVector3d<f64> &a, const SVVector3d<f64> &b) const {
                SVVector3d<f64> bminusa = b - a;
                SVVector3d<f64> cp1 = bminusa.crossProduct(p1 - a);
                SVVector3d<f64> cp2 = bminusa.crossProduct(p2 - a);
                ...
                if (res < 0) {
                    ...
                    SVVector3d<f64> cp1 = bminusa.normalize().crossProduct((p1 - a).normalize());
        ...
        typedef SVTriangle3d<f32> triangle3df;
        
        //! Typedef for an integer 3d triangle.
        typedef SVTriangle3d<s32> triangle3di;
        
    
    }//!namespac util
}//!namespace sv

#endif //__SV_TRIANGLE_3D_H_INCLUDED__
