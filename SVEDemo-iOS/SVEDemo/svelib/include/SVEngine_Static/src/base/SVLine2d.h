...
namespace sv {
    ...
    namespace util{
        ...
        public:
            ...
            SVLine2d() : start(0, 0), end(1, 1) {}
            
            //! Constructor for line between the two points.
            SVLine2d(T xa, T ya, T xb, T yb) : start(xa, ya), end(xb, yb) {}
            
            //! Constructor for line between the two points given as vectors.
            SVLine2d(const SVVector2d<T> &start, const SVVector2d<T> &end) : start(start), end(end) {}
            
            //! Copy constructor.
            SVLine2d(const SVLine2d<T> &other) : start(other.start), end(other.end) {}
            
            // operators
            
            SVLine2d<T> operator+(const SVVector2d<T> &point) const {
                return SVLine2d<T>(start + point, end + point);
            }
            
            SVLine2d<T> &operator+=(const SVVector2d<T> &point) {
                start += point;
                end += point;
                return *this;
            }
            ...
            bool operator==(const SVLine2d<T> &other) const {
            ...
            //! Set this line to new line going through the two points.
            void setLine(const SVVector2d<T> &nstart, const SVVector2d<T> &nend) {
                start.set(nstart);
                end.set(nend);
            }
            
            //! Set this line to new line given as parameter.
            void setLine(const SVLine2d<T> &line) {
            ...
            SVVector2d<T> getMiddle() const {
                return (start + end) / (T) 2;
            }
            
            //! Get the vector of the line.
            /** \return The vector of the line. */
            SVVector2d<T> getVector() const { return SVVector2d<T>(end.X - start.X, end.Y - start.Y); }
            ...
            bool intersectWith(const SVLine2d<T> &l, SVVector2d<T> &out, bool checkOnlySegments = true,
                               bool ignoreCoincidentLines = false) const {
                ...
                if (math::equals(commonDenominator, 0.f)) {
                    ...
                    if (!ignoreCoincidentLines && math::equals(numeratorA, 0.f) &&
                        math::equals(numeratorB, 0.f)) {
                        ...
                        else if (l.start.X > start.X && l.end.X > start.X && l.start.X > end.X &&
                                 l.end.X > end.X)
                            return false;
                        else if (l.start.Y > start.Y && l.end.Y > start.Y && l.start.Y > end.Y &&
                                 l.end.Y > end.Y)
                            return false;
                        else if (l.start.X < start.X && l.end.X < start.X && l.start.X < end.X &&
                                 l.end.X < end.X)
                        ...
                        // else the lines are overlapping to some extent
                            ...
                            if ((start.X > l.start.X && start.X > l.end.X && start.X > end.X) ||
                                ...
                                maxp = end;
                                     ...
                                     (l.start.X > start.X && l.start.X > l.end.X && l.start.X > end.X) ||
                                     (l.start.Y > start.Y && l.start.Y > l.end.Y && l.start.Y > end.Y))
                                maxp = l.start;
                            else
                                maxp = l.end;
                            if (maxp != start &&
                                ((start.X < l.start.X && start.X < l.end.X && start.X < end.X) ||
                                 (start.Y < l.start.Y && start.Y < l.end.Y && start.Y < end.Y)))
                                minp = start;
                            else if (maxp != end &&
                                     ((end.X < l.start.X && end.X < l.end.X && end.X < start.X) ||
                                      (end.Y < l.start.Y && end.Y < l.end.Y && end.Y < start.Y)))
                                minp = end;
                            else if (maxp != l.start && ((l.start.X < start.X && l.start.X < l.end.X &&
                                                          l.start.X < end.X) ||
            ...
            SVVector2d<T> getUnitVector() const {
                T len = (T) (1.0 / getLength());
                return SVVector2d<T>((end.X - start.X) * len, (end.Y - start.Y) * len);
            }
            
            //! Get angle between this line and given line.
            /** \param l Other line for test.
             \return Angle in degrees. */
            f64 getAngleWith(const SVLine2d<T> &l) const {
                SVVector2d<T> vect = getVector();
                SVVector2d<T> vect2 = l.getVector();
            ...
            T getPointOrientation(const SVVector2d<T> &point) const {
            ...
            bool isPointOnLine(const SVVector2d<T> &point) const {
            ...
            bool isPointBetweenStartAndEnd(const SVVector2d<T> &point) const {
            ...
            SVVector2d<T>
            getClosestPoint(const SVVector2d<T> &point, bool checkOnlySegments = true) const {
                SVVector2d<f64> c((f64) (point.X - start.X), (f64) (point.Y - start.Y));
                SVVector2d<f64> v((f64) (end.X - start.X), (f64) (end.Y - start.Y));
                f64 d = v.getLength();
                ...
                if (checkOnlySegments) {
                    if (t < 0) return SVVector2d<T>((T) start.X, (T) start.Y);
                    if (t > d) return SVVector2d<T>((T) end.X, (T) end.Y);
                }
                
                v *= t;
                return SVVector2d<T>((T) (start.X + v.X), (T) (start.Y + v.Y));
            }
        ...
        //    template<>
        //    inline vector2df
        //    SVLine2d<f32>::getClosestPoint(const vector2df &point, bool checkOnlySegments) const {
        ...
        typedef SVLine2d<f32> line2df;
        //! Typedef for an integer line.
        typedef SVLine2d<s32> line2di;
