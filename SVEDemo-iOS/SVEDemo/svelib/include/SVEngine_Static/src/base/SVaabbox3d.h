...
#ifndef SV_AABBOX_3D_H
#define SV_AABBOX_3D_H
...
namespace sv {
    ...
    namespace util {
        ...
        public:
            ...
            //! Default Constructor.
            SVaabbox3d() : MinEdge(-1, -1, -1), MaxEdge(1, 1, 1) {}
            
            //! Constructor with min edge and max edge.
            SVaabbox3d(const SVVector3d<T> &min, const SVVector3d<T> &max) : MinEdge(min),
            MaxEdge(max) {}
            
            //! Constructor with only one point.
            SVaabbox3d(const SVVector3d<T> &init) : MinEdge(init), MaxEdge(init) {}
            
            //! Constructor with min edge and max edge as single values, not vectors.
            SVaabbox3d(T minx, T miny, T minz, T maxx, T maxy, T maxz) : MinEdge(minx, miny, minz),
            ...
            inline bool operator==(const SVaabbox3d<T> &other) const {
            ...
            void reset(const SVaabbox3d<T> &initValue) {
                *this = initValue;
            }
            
            //! Resets the bounding box to a one-point box.
            /** \param initValue New point. */
            void reset(const SVVector3d<T> &initValue) {
                MaxEdge = initValue;
                MinEdge = initValue;
            }
            ...
            void addInternalPoint(const SVVector3d<T> &p) {
            ...
            void addInternalBox(const SVaabbox3d<T> &b) {
            ...
            SVVector3d<T> getCenter() const {
                return (MinEdge + MaxEdge) / 2;
            }
            
            //! Get extent of the box (maximal distance of two points in the box)
            /** \return Extent of the bounding box. */
            SVVector3d<T> getExtent() const {
            ...
            T getVolume() const {
                const SVVector3d<T> e = getExtent();
                return e.X * e.Y * e.Z;
            }
            ...
            void getEdges(SVVector3d<T> *edges) const {
                const SVVector3d <T> middle = getCenter();
                const SVVector3d <T> diag = middle - MaxEdge;
            ...
            SVaabbox3d<T> getInterpolated(const SVaabbox3d<T> &other, f32 d) const {
                f32 inv = 1.0f - d;
                return SVaabbox3d<T>((other.MinEdge * inv) + (MinEdge * d),
            ...
            /** Border is excluded (NOT part of the box)!
             \param p: Point to check.
             \return True if the point is within the box and false if not. */
            bool isPointTotalInside(const SVVector3d<T> &p) const {
                return (p.X > MinEdge.X && p.X < MaxEdge.X &&
                        p.Y > MinEdge.Y && p.Y < MaxEdge.Y &&
                        p.Z > MinEdge.Z && p.Z < MaxEdge.Z);
            }
            ...
            /** \param other: Other box to check against.
             ...
             otherwise false. */
            bool isFullInside(const SVaabbox3d<T> &other) const {
                return (MinEdge.X >= other.MinEdge.X && MinEdge.Y >= other.MinEdge.Y &&
                        MinEdge.Z >= other.MinEdge.Z &&
                        MaxEdge.X <= other.MaxEdge.X && MaxEdge.Y <= other.MaxEdge.Y &&
                        MaxEdge.Z <= other.MaxEdge.Z);
            }
            
            //! Returns the intersection of this box with another, if possible.
            SVaabbox3d<T> intersect(const SVaabbox3d<T> &other) const {
                SVaabbox3d<T> out;
            ...
            /** \param other: Other box to check a intersection with.
             ...
             otherwise false. */
            bool intersectsWithBox(const SVaabbox3d<T> &other) const {
            ...
            bool intersectsWithLine(const SVLine3d <T> &line) const {
            ...
            //轴分离算法 线段与AABB相交
            bool intersectsWithLine(const SVVector3d<T> &linemiddle,
                                    const SVVector3d<T> &linevect,
                                    T halflength) const {
                
                const SVVector3d<T> e = getExtent() * (T) 0.5;
                const SVVector3d<T> t = getCenter() - linemiddle;
            ...
            EIntersectionRelation3D classifyPlaneRelation(const SVPlane3d <T> &plane) const {
                SVVector3d<T> nearPoint(MaxEdge);
                SVVector3d<T> farPoint(MinEdge);
            ...
            //! The near edge
            SVVector3d<T> MinEdge;
            
            //! The far edge
            SVVector3d<T> MaxEdge;
        };
        
        //! Typedef for a f32 3d bounding box.
        typedef SVaabbox3d<f32> aabbox3df;
        //! Typedef for an integer 3d bounding box.
        typedef SVaabbox3d<s32> aabbox3di;

    }//namespace util
    
}//!namespace sv

#endif //__SV_AABBOX_3D_H_INCLUDED__
