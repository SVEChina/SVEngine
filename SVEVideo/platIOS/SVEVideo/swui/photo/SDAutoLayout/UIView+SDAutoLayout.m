...
- (MarginEqualToView)marginEqualToViewBlockWithKey:(NSString *)key
{
    ...
    return ^(UIView *view) {
        ...
        if ([view isKindOfClass:NSClassFromString(@"UITableViewCellContentView")] &&
            ([key isEqualToString:@"equalCenterY"] || [key isEqualToString:@"equalBottom"]))
        {
...
- (void)clearAutoHeigtSettings { [self.sd_bottomViewsArray removeAllObjects]; }
- (void)clearAutoWidthSettings { self.sd_rightViewsArray = nil; }
- (void)updateLayout { [self.superview layoutSubviews]; }
...
- (CGFloat)autoHeight { return [objc_getAssociatedObject(self, _cmd) floatValue]; }
...
- (NSArray *)sd_rightViewsArray { return [[self sd_categoryManager] rightViewsArray]; }
- (void)setSd_rightViewsArray:(NSArray *)sd_rightViewsArray
{
    [[self sd_categoryManager] setRightViewsArray:sd_rightViewsArray];
}

- (CGFloat)sd_bottomViewBottomMargin { return [objc_getAssociatedObject(self, _cmd) floatValue]; }
...
- (CGFloat)sd_rightViewRightMargin { return [[self sd_categoryManager] rightViewRightMargin]; }
@end
...
- (NSNumber *)sd_cornerRadius { return objc_getAssociatedObject(self, _cmd); }
...
- (NSNumber *)sd_cornerRadiusFromWidthRatio { return objc_getAssociatedObject(self, _cmd); }
...
- (NSNumber *)sd_cornerRadiusFromHeightRatio { return objc_getAssociatedObject(self, _cmd); }
...
- (NSArray *)sd_equalWidthSubviews { return objc_getAssociatedObject(self, _cmd); }
...
- (void)setupAutoWidthFlowItems:(NSArray *)viewsArray
           withPerRowItemsCount:(NSInteger)perRowItemsCount
                 verticalMargin:(CGFloat)verticalMargin
               horizontalMargin:(CGFloat)horizontalMagin
{
...
- (void)setupAutoMarginFlowItems:(NSArray *)viewsArray
            withPerRowItemsCount:(NSInteger)perRowItemsCount
                  ...
                  verticalMargin:(CGFloat)verticalMargin
{
    ...
    [self setupAutoWidthFlowItems:viewsArray
             withPerRowItemsCount:perRowItemsCount
...
- (BOOL)isAttributedContent { return [objc_getAssociatedObject(self, _cmd) boolValue]; }
...
- (void)setSingleLineAutoResizeWithMaxWidth:(CGFloat)maxWidth { self.sd_maxWidth = @(maxWidth); }
...
- (void)setupAutoSizeWithHorizontalPadding:(CGFloat)hPadding buttonHeight:(CGFloat)buttonHeight
{
    ...
    self.titleLabel.sd_layout.leftSpaceToView(self, hPadding).topEqualToView(self).heightIs(buttonHeight);
...
- (NSNumber *)fixedWidth { return objc_getAssociatedObject(self, _cmd); }
...
- (NSNumber *)fixedHeight { return objc_getAssociatedObject(self, _cmd); }
...
- (NSNumber *)autoHeightRatioValue { return objc_getAssociatedObject(self, _cmd); }
...
- (NSNumber *)sd_maxWidth { return objc_getAssociatedObject(self, _cmd); }
...
- (UITableView *)sd_tableView { return self.sd_categoryManager.sd_tableView; }
...
- (NSIndexPath *)sd_indexPath { return self.sd_categoryManager.sd_indexPath; }
...
- (SDAutoLayoutModel *)ownLayoutModel { return objc_getAssociatedObject(self, _cmd); }
...
- (BOOL)sd_isClosingAotuLayout { return self.sd_categoryManager.sd_isClosingAotuLayout; }
...
- (void)sd_clearViewFrameCache { self.frame = CGRectZero; }
...
- (void)sd_layoutSubviewsHandle
{
        ...
        [self.sd_categoryManager.flowItems enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
            if (idx < perRowItemsCount)
            {
                if (idx == 0)
                {
                     ...
                     */
                    view.sd_layout.leftSpaceToView(referencedView, 0)
                        .topSpaceToView(referencedView, verticalMargin)
                        .widthIs(w);
                }
                else
                {
                    view.sd_layout.leftSpaceToView(referencedView, horizontalMargin)
            ...
            else
            {
                ...
                view.sd_layout.leftEqualToView(referencedView)
                    .widthIs(w)
                    .topSpaceToView(referencedView, verticalMargin);
            }
        }];
             ...
             (self.sd_bottomViewsArray.count || self.sd_rightViewsArray.count))
    {
        ...
        if (![self isKindOfClass:[UIScrollView class]] && self.sd_rightViewsArray.count &&
            (self.ownLayoutModel.right || self.ownLayoutModel.equalRight))
        {
        ...
        if (![self isKindOfClass:[UIScrollView class]] && self.sd_bottomViewsArray.count &&
            (self.ownLayoutModel.bottom || self.ownLayoutModel.equalBottom))
        {
            ...
            if (label.text.length)
            {
                if (!label.isAttributedContent)
                {
                    CGRect rect = [label.text
                        boundingRectWithSize:CGSizeMake(label.width_sd, MAXFLOAT)
...
- (void)layoutAutoWidthWidthView:(UIView *)view model:(SDAutoLayoutModel *)model
{
    if ([view isKindOfClass:[UILabel class]])
    {
        ...
        if (label.text.length)
        {
            ...
            {
                CGRect rect = [label.text
                    boundingRectWithSize:CGSizeMake(width, label.height_sd)
                                 options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                              attributes:@{
                                  NSFontAttributeName : label.font
                              }
...
- (void)layoutTopWithView:(UIView *)view model:(SDAutoLayoutModel *)model
{
    if (model.top)
    {
        ...
        {
            if (!view.fixedHeight)
            {  // view.autoTop && view.autoBottom && view.bottom
...
- (void)setupCornerRadiusWithView:(UIView *)view model:(SDAutoLayoutModel *)model
{
    ...
    if (view.sd_cornerRadius && (cornerRadius != [view.sd_cornerRadius floatValue]))
    {
        newCornerRadius = [view.sd_cornerRadius floatValue];
             ...
             (cornerRadius != [view.sd_cornerRadiusFromWidthRatio floatValue] * view.width_sd))
    {
        newCornerRadius = view.width_sd * [view.sd_cornerRadiusFromWidthRatio floatValue];
    }
    else if (view.sd_cornerRadiusFromHeightRatio &&
             (cornerRadius != view.height_sd * [view.sd_cornerRadiusFromHeightRatio floatValue]))
    {
...
- (void)addAutoLayoutModel:(SDAutoLayoutModel *)model { [self.autoLayoutModelsArray addObject:model]; }
@end
...
- (BOOL)shouldReadjustFrameBeforeStoreCache { return self.sd_categoryManager.shouldReadjustFrameBeforeStoreCache; }
...
- (CGFloat)left_sd { return self.frame.origin.x; }
...
- (CGFloat)top_sd { return self.frame.origin.y; }
...
- (CGFloat)right_sd { return self.frame.origin.x + self.frame.size.width; }
...
- (CGFloat)bottom_sd { return self.frame.origin.y + self.frame.size.height; }
...
- (CGFloat)centerX_sd { return self.left_sd + self.width_sd * 0.5; }
- (void)setCenterX_sd:(CGFloat)centerX_sd { self.left_sd = centerX_sd - self.width_sd * 0.5; }
- (CGFloat)centerY_sd { return self.top_sd + self.height_sd * 0.5; }
- (void)setCenterY_sd:(CGFloat)centerY_sd { self.top_sd = centerY_sd - self.height_sd * 0.5; }
- (CGFloat)width_sd { return self.frame.size.width; }
...
- (CGFloat)height_sd { return self.frame.size.height; }
...
- (CGPoint)origin_sd { return self.frame.origin; }
...
- (CGSize)size_sd { return self.frame.size; }
- (void)setSize_sd:(CGSize)size_sd { [self setSize:size_sd]; }
...
- (CGFloat)left { return self.left_sd; }
- (void)setLeft:(CGFloat)left { self.left_sd = left; }
- (CGFloat)right { return self.right_sd; }
- (void)setRight:(CGFloat)right { self.right_sd = right; }
- (CGFloat)width { return self.width_sd; }
- (CGFloat)height { return self.height_sd; }
- (CGFloat)top { return self.top_sd; }
- (void)setTop:(CGFloat)top { self.top_sd = top; }
- (CGFloat)bottom { return self.bottom_sd; }
- (void)setBottom:(CGFloat)bottom { self.bottom_sd = bottom; }
- (CGFloat)centerX { return self.centerX_sd; }
- (void)setCenterX:(CGFloat)centerX { self.centerX_sd = centerX; }
- (CGFloat)centerY { return self.centerY_sd; }
- (void)setCenterY:(CGFloat)centerY { self.centerY_sd = centerY; }
- (CGPoint)origin { return self.origin_sd; }
- (void)setOrigin:(CGPoint)origin { self.origin_sd = origin; }
- (CGSize)size { return self.size_sd; }
@end
