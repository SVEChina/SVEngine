...
cptr8 SVglTFio::_normalize_path(cptr8 _filename) {
...
cptr8 SVglTFio::_get_dirname(cptr8 _filename) {
    SVString filename = _normalize_path(_filename);
...
cptr8 SVglTFio::_get_extension(cptr8 _filename) {
    SVString filename = _normalize_path(_filename);
    auto pos      = filename.rfind('.');
    if (pos == -1) return "";
...
cptr8 SVglTFio::_get_filename(cptr8 _filename) {
    SVString filename = _normalize_path(_filename);
    auto pos      = filename.rfind('/');
    if (pos == -1) return "";
    return SVString::substr(filename.c_str(), pos + 1).c_str();
...
// Replace extension.
cptr8 SVglTFio::_replace_extension(cptr8 _filename, cptr8 _ext) {
    SVString filename = _normalize_path(_filename);
    SVString ext      = _normalize_path(_ext);
...
bool SVglTFio::_exists_file(cptr8 _filename) {
...
// -----------------------------------------------------------------------------
// Load a JSON object
json SVglTFio::_load_json(cptr8 _filename) {
...
// Load a scene
svgltf_scene* SVglTFio::load_scene(
                         cptr8 _filename, bool _load_textures, bool _skip_missing) {
    cptr8 ext = _get_extension(_filename);
    if (strcmp(ext, "gltf") == 0 || strcmp(ext, "GLTF") == 0) {
        return _load_gltf_scene(_filename, _load_textures, _skip_missing);
    }else {
...
bool SVglTFio::_gltf_to_scene(svgltf_scene* _scene, const json& _gltf, cptr8 _dirname) {
    // convert textures
    ...
    if (_gltf.count("buffers")) {
        ...
        for (auto bid = 0; bid < _gltf.at("buffers").size(); bid++) {
            ...
            if (startswith(uri, "data:")) {
                ...
                auto filename = _normalize_path((SVString(_dirname) + "/" + uri).c_str());
...
// Load a scene
svgltf_scene* SVglTFio::_load_gltf_scene(cptr8 _filename, bool _load_textures, bool _skip_missing) {
    // initialization
    svgltf_scene *scene = new svgltf_scene();
    // convert json
    auto js = _load_json(_filename);
    if (js.empty()) return nullptr;
    try {
        if (!_gltf_to_scene(scene, js, _get_dirname(_filename)))
            return nullptr;
    } catch (...) {
        return nullptr;
    }
    ...
    // load textures
    auto dirname = _get_dirname(_filename);
    if (_load_textures) {
    ...
    // fix scene
    scene->name = _get_filename(_filename);
