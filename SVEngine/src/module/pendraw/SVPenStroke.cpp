...
:SVGameBase(_app) {
    ...
    //
    m_pGlowVertData = MakeSharedPtr<SVDataSwap>();
    m_pGlowMesh = _app->getRenderMgr()->createMeshRObj();
...
SVPenStroke::~SVPenStroke() {
    ...
    m_pVertData->reback();
    m_pVertData = nullptr;
    m_pGlowVertData->reback();
    m_pGlowVertData = nullptr;
...
void SVPenStroke::_genMesh() {
    ...
    m_glowVertexNum = t_pt_original_num*36;
    //三维盒子网格
    s32 t_pt_num = m_ptPool.size();
...
void SVPenStroke::_genGlow(FVec3 &_pt){
    ...
    t_ver[5+6*i].a = t_a;
    //
    m_pGlowVertData->appendData(t_ver, 36*sizeof(V3_C_T0));
}
...
void SVPenStroke::_drawGlow(){
    ...
    if (t_renderer && t_rs && m_pRenderObj && m_pGlowMesh && m_glowVertexNum > 0) {
        if (!m_pGlowMtl) {
            m_pGlowMtl = MakeSharedPtr<SVMtlStrokeBase>(mApp, "penstroke_base");
            m_pGlowMtl->setTexture(0, m_pTex);
        ...
        //更新顶点数据
        m_pGlowMesh->setVertexDataNum(m_glowVertexNum);
        m_pGlowMesh->setVertexData(m_pGlowVertData);
