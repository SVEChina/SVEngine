...
void main()
{
    ...
    float red     =   texture2D(    aTexture0,     vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
    float green =     texture2D(    aTexture0,     vec2(uv.x + xOffset,      y)).g+staticVal;
    float blue     =    texture2D(    aTexture0,     vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
